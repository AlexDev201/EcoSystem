...
export const MetricCards = ({ latestReading, alerts }: MetricCardsProps) => {
  // Cast explícito a Reading[] para que TS no infiera 'unknown'
  const allReadings = Object.values(latestReading).filter(
    (r): r is Reading => r != null
  )

  const totalPower = allReadings.reduce((sum, r) => sum + (r.power ?? 0), 0)
  const avgTemp = allReadings.length
    ? allReadings.reduce((sum, r) => sum + (r.temperature ?? 0), 0) / allReadings.length
    : 0
